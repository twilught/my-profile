...
// ถ้าเซ็ต tsconfig paths เป็น "@/data/*" ใช้บรรทัดล่างแทน
    ...
    <main className="mx-auto max-w-7xl px-6 py-12">
      <header className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="opacity-70 mt-1">
            {filtered.length} รายการ{q ? ` • ค้นหา: “${q}”` : ""}{activeTag ? ` • แท็ก: ${activeTag}` : ""}
          </p>
        </div>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        ...
        {allTags.map((t) => {
          ...
          const href = new URLSearchParams({ ...(q ? { q } : {}), tag: t }).toString();
          return (
            <Link
              key={t}
              href={`/projects?${href}`}
        ...
        {filtered.map((p) => (
          ...
          >
            ...
            <div className="p-5">
              ...
              <div className="mt-4 flex items-center gap-2 text-sm opacity-60">{p.date}</div>

              <div className="mt-4 flex items-center gap-2">
                {/* ลิงก์ไปหน้ารายละเอียด (ให้ตรงเส้นทางจริง) */}
                <Link
